 ...
 * @param {Number} [options.type=0] Asset type as defined by `ModuleInfo.AssetType`.
 * @returns {Promise<Buffer>}
 */
async function createAssetModule(data, name, options) {
	...
	if (options?.type !== undefined && options.type !== ModuleInfo.AssetType.DEFAULT) {
		extensions.push({
			type: ModuleInfo.ModuleInfoExtension.ASSET_TYPE,
 ...
 * @param {String|Buffer} [opts.snapshotHash] Snapshot hash. Can be a buffer or hex-encoded string.
 *        If not provided, the hash will be omitted in the asset data.
 * @param {String|Number} [opts.updatedAt] Snapshot creation time. Can be a string in a format
 *        supported by `Date.parse()`, or a number of milliseconds elapsed since the UNIX epoch.
 *        If not provided, the timestamp will be omitted in the asset data.
 * @param {Boolean} [opts.override=false] If `true`, the asset module will be marked as containing
 *        overridden variables.
 * @returns {Promise<Buffer>} Module binary.
 */
async function createEnvVarsAssetModule(vars, { snapshotHash, updatedAt, override } = {}) {
	const maxAssetSize = 16 * 1024;

	// Validate the arguments
	if (!vars || typeof vars !== 'object') {
		throw new Error('Expected an object with variable values');
	}
	let approxSize = 0;
	...
	const assetData = EnvVars.encode(EnvVars.create({
		vars: Object.entries(vars).map(([name, value]) => EnvVars.Var.create({ name, value })),
		hash: snapshotHash,
		updatedAt
	})).finish();

	const assetModule = await createAssetModule(assetData, 'env-vars', {
		type: override ? ModuleInfo.AssetType.ENV_VARS_OVERRIDE : ModuleInfo.AssetType.ENV_VARS_USER,
		compress: false
	});
