...
function firmwareModuleInfoForPlatformAndFunction(id, func, index) {
	...
	for (let m of p.firmwareModules) {
		if (m.type === moduleFunctionToString(func)) {
		...
		if (prefix.extensions) {
			...
			for (let ext of prefix.extensions) {
				if (ext.data) {
					...
					if (expectedSize === 0) {
						expectedSize = calculateModulePrefixSize(prefix);
						if (expectedSize !== prefix.prefixSize) {
							throw new RangeError(`Extensions will not fit current prefix without resizing`);
						}
					}
...
function encodeModuleExtension(extension) {
	...
	switch (extension.type) {
		...
		case ModuleInfo.ModuleInfoExtension.NAME: {
			const name = Buffer.from(extension.name, 'ascii');
	...
	if (suffix.extensions) {
		...
		for (let ext of suffix.extensions) {
			if (ext.data) {
				...
				if (expectedSize === 0) {
					expectedSize = calculateModuleSuffixSize(suffix);
...
async function updateModuleAssetDependencies(data, assets) {
	...
	assets = await Promise.all(assets.map(async (asset) => {
		...
		if (!hash && asset.data) {
			...
			let moduleInfo;
			try {
				const { prefixInfo: prefix } = await parser.parsePrefix({ fileBuffer: asset.data });
				const { suffixInfo: suffix } = await parser.parseSuffix({ fileBuffer: asset.data });

				if (suffix.crc && suffix.crc.ok) {
					// Looking like a valid module
					moduleInfo.prefix = prefix;
					moduleInfo.suffix = suffix;
				}
			} catch (error) {
				// Ignore
			}
			let parsed = false;
			if (moduleInfo) {
				if (moduleInfo.prefix.moduleFunction === ModuleInfo.FunctionType.ASSET && moduleInfo.suffix.extensions) {
	...
	if (extensionsInPrefix && extensions.length > 0) {
		if (extensions[extensions.length - 1].type !== ModuleInfo.ModuleInfoExtension.END) {
			extensions.push({
...
async function createAssetModule(data, name, options) {
	...
	if (!options || options.compress === undefined || options.compress) {
		buffer = await compressModule(buffer);
	}
...
async function unwrapAssetModule(data) {
	...
	({ prefixInfo: prefix } = await parser.parsePrefix({ fileBuffer: data }));
	({ suffixInfo: suffix } = await parser.parseSuffix({ fileBuffer: data }));
	if (prefix.moduleFunction !== ModuleInfo.FunctionType.ASSET) {
		throw new RangeError('Invalid module function type');
	}
	...
	if (assets) {
		for (let asset of assets) {
			if (typeof asset === 'string') {
				const data = Buffer.from(await fsAsync.readFile(asset));
				asset = {
					data: data,
