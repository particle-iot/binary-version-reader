...
async function transformModule(data, transform, options) {
	...
	// Sanity checks
	if (suffix.suffixSize < MIN_MODULE_SUFFIX_SIZE || suffix.suffixSize > MAX_MODULE_SUFFIX_SIZE) {
		throw new RangeError('Invalid suffix size');
	}
	if (data.length < prefix.prefixOffset + MODULE_PREFIX_SIZE + suffixSize + 4 /* CRC-32 */) {
		throw new RangeError('Invalid size of the module data');
	}
	if ((prefix.moduleFlags & ModuleFlags.DROP_MODULE_INFO) && prefix.prefixOffset !== 0) {
		throw new RangeError('DROP_MODULE_INFO can\'t be set for a module with a vector table');
	}
	// Extract and update individual sections of the module binary
	let vectorTable = data.slice(0, prefix.prefixOffset);
	let firmwareData = data.slice(prefix.prefixOffset + MODULE_PREFIX_SIZE, prefix.prefixOffset + MODULE_PREFIX_SIZE +
			data.length - prefix.prefixOffset - MODULE_PREFIX_SIZE - suffix.suffixSize - 4 /* CRC-32 */);
	if (typeof transform === 'function') {
		const r = await transform({ vectorTable, prefix, suffix, compressedHeader, firmwareData, });
		...
		if (r.firmwareData !== undefined) {
			firmwareData = r.firmwareData;
		}
		if (r.vectorTable !== undefined) {
			vectorTable = r.vectorTable;
		}
	} else if (transform) {
	...
	const destBuf = Buffer.alloc(vectorTable.length + MODULE_PREFIX_SIZE + firmwareData.length + suffix.suffixSize + 4 /* CRC-32 */);
	let offs = 0;
	offs += vectorTable.copy(destBuf, offs); // Vector table
	prefix.prefixOffset = vectorTable.length;
	updateModulePrefix(destBuf, prefix); // Module prefix
	offs += MODULE_PREFIX_SIZE;
	offs += firmwareData.copy(destBuf, offs); // Firmware data
	updateModuleSuffix(destBuf, suffix); // Module suffix
	// Update the checksums
	if (!options || options.updateSha256 === undefined || options.updateSha256) {
		updateModuleSha256(destBuf);
	}
