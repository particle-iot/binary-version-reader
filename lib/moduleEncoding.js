 ...
 *        containing user variables as opposed to a snapshot of overridden variables.
...
async function createEnvVarsAssetModule(vars, snapshot = undefined) {
	...
	if (snapshot) {
		if (typeof snapshot !== 'object') {
			throw new Error('Expected snapshot details to be an object');
		}
		hash = snapshot.hash;
		if (!Buffer.isBuffer(hash)) {
			hash = Buffer.from(hash, 'hex');
		}
		if (hash.length !== 32) { // SHA-256
	...
	const assetModule = await createAssetModule(assetData, 'env-vars', {
		type: snapshot ? ModuleInfo.AssetType.ENV_VARS_SNAPSHOT : ModuleInfo.AssetType.ENV_VARS_USER,
		compress: false
	});
