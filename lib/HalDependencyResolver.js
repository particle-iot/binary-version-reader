...
--> therefore, we need a step, post solve, that grabs our binary, and does
	 ...
	 * @param Promise
	 */
	descibeHasMissingDependencies: function(describe) {
		if (!Array.isArray(describe.m)) {
			return when.reject('no modules in describe message');
		}
		var modules = describe.m.map(function(item) {
			return new FirmwareModule(item);
		});
		var userModule = modules.reduce(function(previous, item) {
			if (previous) {
				return previous;
			}
			return item.isUserModule() && item.location === 'm' ? item : null;
		}, null);
...
//		}
	...
	_getModuleFirstDependecy: function(modules, currentModule) {
		...
		if (currentModule.dependencies.length === 0) {
			...
			currentModule.dependencies.map(function(missing){
