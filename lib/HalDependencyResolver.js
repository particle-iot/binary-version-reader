		...
		return pipeline([
			...
			function(updates) {
				...
				for(var i = 0; i < updates.length; i++) {
					var extras = that.resolve_dependencies(describe, updates[i]);
					if (extras && (extras.length > 0)) {
						that._array_insert(updates, extras);
					}
				}
...
--> therefore, we need a step, post solve, that grabs our binary, and does a dependency check on it,
	...
	assimilateModule: function(filename) {
		...
		return parser.parseFile(filename)
			.then(
			function(fileInfo) {
				fileInfo.describe = that._binary_to_describe(fileInfo);
	...
	retrieveModules: function(modules) {
		...
		for(var i=0;i<modules.length;i++) {
			...
			for(var a=0;a<this._moduleStorage.length;a++) {
				var avail = this._moduleStorage[a];
	...
	solve: function(device_modules, binary_info) {
		...
		var safe_binary_requires = this._binary_to_describe(binary_info);
