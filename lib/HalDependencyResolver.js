...
HalDependencyResolver.prototype = {
	 ...
	 * @returns Promise
	 */
	parseAndResolve: async function(describe, fileBuffer) {
		...
		await parser.parseBuffer(fileInfo);
		return await this.resolveDependencies(describe, fileInfo);
	},
	 ...
	 * @returns Promise
	 */
	resolveDependencies: async function(describe, fileInfo) {
		if (!describe || !util.isArray(describe)) {
			throw new Error('wrong describe payload given');
		}

		if (!fileInfo) {
			throw new Error('no fileInfo provided');
		}

		console.log('resolving dependencies for ', fileInfo.filename);

		let modules = await this.solve(describe, fileInfo.prefixInfo);
		let updates = [];
		if (modules && (modules.length > 0)) {
			updates = await this.retrieveModules(modules);
		}
		...
		for (var i = 0; i < updates.length; i++) {
			var promise = this.resolveDependencies(describe, updates[i]);
...
--> therefore, we need a step, post solve, that grabs our binary, and does
	 ...
	 * @returns Boolean
	 */
	assimilateModule: async function(filename) {
		...
		try {
			...
			fileInfo.describe = this._binaryToDescribe(fileInfo.prefixInfo);
			this._moduleStorage.push(fileInfo);
		} catch (error) {
	 ...
	 * @returns Object
	 */
	retrieveModules: async function(modules) {
	 ...
	 * @param Promise
	 */
	userModuleHasMissingDependencies: async function(describe) {
		if (!Array.isArray(describe.m)) {
			throw new Error('no modules in describe message');
		}
		...
		for(var i=0;i<userModule.d.length;i++) {
			...
			if (deps && (deps.length > 0)) {
				...
				throw [new FirmwareModule(deps[0])];
			}
		}
