...
HalModuleParser.prototype = {
	...
	_readSuffix: function(fileBuffer) {
		...
		if (suffixSize > MIN_MODULE_SUFFIX_SIZE + 4) {
			...
			while (idx > 0 && idx < fileBuffer.length - 4) {
				const type = fileBuffer.readUInt16LE(idx);
				idx += 2;
				const length = fileBuffer.readUInt16LE(idx);
				idx += 2;
				if (type === ModuleInfo.ModuleInfoExtension.END || type === ModuleInfo.ModuleInfoExtension.INVALID) {
					break;
				}
				if (idx + length >= fileBuffer.length) {
					break;
				}
				const offset = idx - 4;
				extensions.push({
