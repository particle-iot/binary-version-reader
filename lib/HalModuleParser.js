...
/**
 * Understands how to parse out the header and footer metadata sections of the newer hal firmware for the
...
HalModuleParser.prototype = {
	...
	/**
	 * broken out if you're working with a buffer instead, expects a "fileInfo" object with the
	...
	/**
	 * goes and reads out the file if it exists
	 * @param filename
	 * @returns {*}  a deferred with the fileBuffer
	 * @private
	 */
	 ...
	 *
	 * This changes based on whether or not we're in the bootloader, and we attempt to validate the result
	 * based on the assumption that firmware addresses tend to start with hex "80" since they're stored after
	 * 0x0800xxxx, etc.
	 *
	 * @param fileBuffer
	 * @returns {number}
	...
	/**
	 * parses out the prefix area of the binary, after attempting to determine the correct offset
	 * @param fileBuffer
	...
	/**
	 * parses out the suffix area of the binary, only reads back into the file as far
	 * as is provided by the "suffixSize" value
	 *
	 * @param fileBuffer
	 * @returns {{product_id: (*|Number), product_version: (*|Number), fw_unique_id: (*|String), reserved: (*|Number), suffixSize: (*|Number), crcBlock: (*|String)}}
	 * @private
	 */
	_readSuffix: function(fileBuffer) {
		...
		idx -= 32;
		var fw_unique_id = fileBuffer.slice(idx, idx + 32).toString('hex');

		idx -= 2;
		var reserved = fileBuffer.readUInt16LE(idx);

		idx -= 2;
		var product_version = fileBuffer.readUInt16LE(idx);

		idx -= 2;
		var product_id = fileBuffer.readUInt16LE(idx);
