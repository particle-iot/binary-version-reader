...
var MINIMUM_MODULE_SUFFIX_SIZE = moduleInfo.MINIMUM_MODULE_SUFFIX_SIZE;
...
HalModuleParser.prototype = {
	...
	_readSuffix: function(fileBuffer) {
		...
		// This doesn't make a lot of sense, but just to preserve backward compatibility:
		//
		// 1. Some modules seem to have the suffix data filled with 0xff (see, e.g., RC4_bootloader_pad_BM-09.bin).
		//    In this case, we read and report the product ID, product version and suffix size fields as 0xffff.
		//
		// 2. If the module suffix doesn't include the product ID and product version fields (i.e. the suffix size
		//    is less than 40 bytes), we set them to -1.
		//
		...
		if (suffixSize < MINIMUM_MODULE_SUFFIX_SIZE + 4) {
			productVersion = -1;
			productId = -1;
		} else if (suffixSize === MINIMUM_MODULE_SUFFIX_SIZE + 6) {
			idx -= 2;
			var productIdHigh = fileBuffer.readUInt16LE(idx);
			productId |= productIdHigh << 16;
		}
